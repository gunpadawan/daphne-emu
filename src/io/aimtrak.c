...
static int event_test_bit(unsigned bit, unsigned char* evtype_bitmask)
{
...
static void event_log(int f, unsigned char* evtype_bitmask)
{
	int version;
...
static int event_test_bit_feature(int f, unsigned bit, unsigned char* evtype_bitmask, const int* feature)
{
	unsigned i;
...
static int event_is_joystick(int f, unsigned char* evtype_bitmask)
{
    int feat[] = {BTN_LEFT, -1};
    return (event_test_bit_feature(f, EV_ABS, evtype_bitmask, ABS_FEATURE) &&
            ...
            !event_test_bit_feature(f, EV_KEY, evtype_bitmask, feat) );
}

static int event_is_mouse(int f, unsigned char* evtype_bitmask)
{
    int feat[] = {BTN_LEFT, -1};
    return ((event_test_bit_feature(f, EV_ABS, evtype_bitmask, ABS_FEATURE) ||
             event_test_bit_feature(f, EV_REL, evtype_bitmask, REL_FEATURE) ) &&
...
static int event_open(const char* file, unsigned char* evtype_bitmask, unsigned evtype_size)
{
    int f;
...
static int event_read(int f, int* type, int* code, int* value)
{
	int size;
...
static int init_buttons(struct dev_context* dev_p)
{
    unsigned char key_bitmask[KEY_MAX/8 + 1];
    int i;
    dev_p->nbBtns = 0;
    ...
    memset(key_bitmask, 0, sizeof(key_bitmask));
    if (ioctl(dev_p->devFd, EVIOCGBIT(EV_KEY, sizeof(key_bitmask)), key_bitmask) < 0) {
        log_std(("event: error in ioctl(EVIOCGBIT(EV_KEY,%d))\n", (int)KEY_MAX));
        return 0;
    }

    for(i=0 ; i<KEY_MAX && dev_p->nbBtns<MAX_BTN ; ++i) {
        if (event_test_bit(i, key_bitmask)) {
            dev_p->btns[dev_p->nbBtns].code = i;
            dev_p->btns[dev_p->nbBtns].state = 0;
            dev_p->nbBtns++;
        }
    }
...
static int init_axes(struct dev_context* dev_p)
{
    unsigned char abs_bitmask[ABS_MAX/8 + 1];

    memset(abs_bitmask, 0, sizeof(abs_bitmask));
    if (ioctl(dev_p->devFd, EVIOCGBIT(EV_ABS, sizeof(abs_bitmask)), abs_bitmask) < 0) {
        log_std(("event: error in ioctl(EVIOCGBIT(EV_ABS,%d))\n", (int)ABS_MAX));
        return 0;
    }
    ...
    int axeId;
    int features[5];
    struct axe_context* axe;

    for (axeId=0 ; axeId<2 ; ++axeId) {
        axe = &dev_p->axes[axeId];

        if (event_test_bit(axeId, abs_bitmask)) {
            memset(features, 0, sizeof(features));
            if (ioctl(dev_p->devFd, EVIOCGABS(axeId), features) >= 0) {
...
int event_locate(int vendor, int product, int isMouseNotJoystick, struct dev_context* dev_p)
{
    ...
    for( i=0 ; i<EVENT_MAX ; ++i)
    {
        ...
        dev_p->devFd = event_open(file, evtype_bitmask, sizeof(evtype_bitmask));
        if (dev_p->devFd == -1) {
            if (errno == EACCES) {
                log_std(("event: can't open device %s\n", file));
			}
			continue;
		}

#ifdef DEBUG
        event_log(dev_p->devFd, evtype_bitmask);
#endif
        if (ioctl(dev_p->devFd, EVIOCGID, &device_info)) {
			log_std(("event: error in ioctl(EVIOCGID)\n"));
            event_close(dev_p);
			continue;
		}
        ...
        /* test device */
        if( ( (vendor == -1) || (device_info[ID_VENDOR] == (unsigned short)vendor) ) &&
            ( (product == -1) || (device_info[ID_PRODUCT] == (unsigned short)product) ) &&
            ( (isMouseNotJoystick && event_is_mouse(dev_p->devFd, evtype_bitmask) ||
              (!isMouseNotJoystick && event_is_joystick(dev_p->devFd, evtype_bitmask)) ) ) )
        {
            /* we have found our device */
            if (init_axes(dev_p) && init_buttons(dev_p))
                return 1;
        }
...
{
    int type, code, value;
    while(event_read(dev_p->devFd, &type, &code, &value) == 0) {
        if (EV_ABS == type) {
            dev_p->axes[code].value = value;
        }
        else if (EV_KEY == type) {
            int i;
            for(i=0 ; i<dev_p->nbBtns ; ++i) {
                if(dev_p->btns[i].code == code) {
                    dev_p->btns[i].state = (value != 0);
                    break;
                }
            }
        }
...
int main(int argc, char** argv)
{
    ...
    ret = event_locate(0xd209, 0x1602, 1, &device);
    if(ret) {
        printf("device is %d\n", device.devFd);
